...
public class ProductoServiceImpl implements ProductoServicePort {
    ...
    @Override
    public List<ProductoDto> findProductoDtoById(Integer id) {
        Message message = new Message();
        List<ProductoDto> productoDtoList = new ArrayList<ProductoDto>();
        ProductoDto listProductoDTO = new ProductoDto();
        listProductoDTO =  productoPersistencePort.getProductoDtoById(id);
        if(listProductoDTO==null){
            message.setEcho (StringResponse.ErrorNOHAYPRODUCTO.getName());
            message.setCode( StringResponse.ErrorNOHAYPRODUCTO.getCode());
           throw new IllegalStateException("Error : " + message.toString());
        }
        productoDtoList.add(listProductoDTO);
